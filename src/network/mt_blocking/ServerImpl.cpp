...
ServerImpl::ServerImpl(std::shared_ptr<Afina::Storage> ps, std::shared_ptr<Logging::Service> pl)
    : Server(ps, pl), _max_workers(3), _cur_workers(0) {}
...
void ServerImpl::clientOnRun(int client_socket) {
    ...
    try {
        ...
        while ((readed_bytes = read(client_socket, client_buffer + remaining_bytes, sizeof(client_buffer) - remaining_bytes)) > 0) {
    ...
    std::lock_guard<std::mutex> l_guard(_mutex);
    _cur_workers--;
    if(_cur_workers == 0 && !running.load())
        cv.notify_all();
}
...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        // Configure read timeout
        {
            ...
            {
                std::lock_guard<std::mutex> l_guard(_mutex);
                if (_cur_workers < _max_workers / 2) {
                    tv.tv_sec = 3;
                    tv.tv_usec = 0;
                } else if (_cur_workers < 3 * _max_workers / 4) {
                    tv.tv_sec = 2;
                    tv.tv_usec = 0;
                } else {
                    tv.tv_sec = 0;
        ...
        // TODO: Start new thread and process data from/to connection
        {
            ...
            */
            ...
            if (_cur_workers == _max_workers) {
                std::string msg = "Max threads";
                send(client_socket, msg.data(), msg.size(), 0);
                close(client_socket);
            } else {
                _cur_workers++;
                std::thread Worker(&ServerImpl::clientOnRun, this, client_socket);
                Worker.detach();
            }
        }
    }
